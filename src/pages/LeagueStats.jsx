...
function LeagueStats({ fplData }) {
  ...
  const filterPlayers = (players) => {
    ...
    if (viewFilter !== "All players") {
      filtered = filtered.filter(
        (player) => player.position === POSITION_MAP[viewFilter]
      );
    }
  ...
  const stats = fplData.elements.map((player) =>
        ...
        >
          <thead>
            <tr style={{ backgroundColor: "#37003c", color: "white" }}>
              ...
              <th
                onClick={() => handleSort("selectedBy")}
                style={getHeaderStyle("selectedBy")}
              >
                Selected
          ...
          <tbody>
            {displayedPlayers.map((player) => (
              <tr key={player.id} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td style={tableCellStyle}>
                  £{formatValue(player.stats.cost, 1)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.selectedBy, 1)}%
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.form, 1)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.points)}
                </td>
                <td style={tableCellStyle}>{formatValue(player.stats.xPts)}</td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.minutes)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.goals)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.assists)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.cleanSheets)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.xG, 2)}
                </td>
                <td style={tableCellStyle}>
                  {formatValue(player.stats.xA, 2)}
                </td>
              </tr>
            ))}
          </tbody>
